...
export default function Home() {
  return (
    <div className="bg-cream min-h-screen">
      <main>
        {/* ── HERO ───────────────────────────────────────────────────────────── */}
        <section className="pt-20 lg:pt-28 bg-cream">
          <div className="max-w-7xl mx-auto px-5 sm:px-8">
            <div className="grid lg:grid-cols-2 gap-10 xl:gap-20 items-center min-h-[90vh] py-16 lg:py-0">
              ...
              <div className="flex flex-col gap-7 lg:gap-8 order-2 lg:order-1">
                ...
                <div className="flex flex-col gap-4">
                  <h1 className="font-serif text-[2.6rem] sm:text-5xl lg:text-[3.4rem] xl:text-[4rem] text-espresso leading-[1.08] font-normal">
                    Curadoria de sabores, marcas e histórias.
                  </h1>
                  <p className="font-serif text-xl sm:text-2xl text-espresso/55 italic font-normal">
                    Descubra quem faz.
                  </p>
                </div>
                <p className="font-sans text-[0.92rem] text-espresso/65 leading-relaxed max-w-sm">
                ...
                <div className="flex flex-wrap gap-3 pt-1">
                  ...
                  <a
                    href="#"
                    className="inline-flex items-center gap-2 border border-espresso/25 text-espresso font-sans text-[0.72rem] font-semibold tracking-[0.18em] uppercase px-7 py-4 hover:border-espresso/60 transition-colors"
                  >
              ...
              <div className="relative order-1 lg:order-2 flex justify-center lg:justify-end">
                <div className="relative w-full max-w-sm lg:max-w-none">
                  ...
                  {/* Decorative accents */}
                  <div className="absolute -bottom-3 -left-3 w-20 h-20 bg-sand -z-10" />
                  <div className="absolute -top-3 -right-3 w-14 h-14 bg-beige/50 -z-10" />
                </div>
              </div>
        ...
        <section className="bg-sand py-12 lg:py-16">
          <div className="max-w-7xl mx-auto px-5 sm:px-8">
            <div className="flex items-center justify-between mb-8">
              <h2 className="font-serif text-xl lg:text-2xl text-espresso">Explorar por categoria</h2>
              <a
                href="/produtos"
                className="font-sans text-[0.68rem] text-caramel tracking-[0.2em] uppercase font-semibold hover:text-terracota transition-colors flex items-center gap-1.5"
              >
                Ver todas <ArrowRight size={11} />
              </a>
            </div>
            <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide -mx-5 px-5 sm:-mx-8 sm:px-8 lg:mx-0 lg:px-0 lg:grid lg:grid-cols-7">
              {CATEGORIES.map(({ name, Icon }) => (
                ...
                >
                  <div className="w-[4.5rem] h-[4.5rem] sm:w-20 sm:h-20 lg:w-full bg-cream group-hover:bg-beige/70 transition-colors border border-beige/40 flex items-center justify-center">
                    <Icon size={26} strokeWidth={1.4} className="text-espresso/70 group-hover:text-espresso transition-colors" />
                  </div>
                  <span className="font-sans text-[0.68rem] font-medium text-espresso/75 tracking-wide text-center w-[4.5rem] sm:w-20 lg:w-full leading-tight">
                    {name}
                  </span>
                </a>
              ))}
              ...
              {PRODUCERS.map((p) => (
                <div key={p.id} className="group flex flex-col bg-sand hover:bg-sand/70 transition-colors">
                  ...
                  <div className="p-5 flex flex-col gap-3 flex-1">
                    <div>
                      <span className="font-sans text-[0.62rem] tracking-[0.28em] uppercase text-caramel font-semibold">
                        {p.category}
                      </span>
                      <h3 className="font-serif text-[1.25rem] text-espresso mt-1 font-normal">{p.name}</h3>
                    </div>
                    <div className="flex items-center gap-1.5 text-espresso/45">
                      <MapPin size={11} strokeWidth={1.8} />
                      <span className="font-sans text-[0.72rem]">{p.city}</span>
                    </div>
                    <p className="font-sans text-[0.83rem] text-espresso/65 leading-relaxed flex-1">
                      {p.bio}
                    </p>
                    <a
                      href="#"
                      className="font-sans text-[0.7rem] font-semibold tracking-[0.18em] uppercase text-espresso border-b border-espresso/25 pb-px self-start hover:border-terracota hover:text-terracota transition-colors mt-1"
                    >
                      Conhecer →
                    </a>
                  </div>
        ...
        {/* ── BANNER ─────────────────────────────────────────────────────────── */}
        <section className="bg-espresso py-20 lg:py-32">
          <div className="max-w-5xl mx-auto px-5 sm:px-8 text-center flex flex-col items-center gap-8">
            <span className="font-sans text-[0.65rem] tracking-[0.35em] uppercase text-cream/35 font-semibold">
              Nossa missão
            </span>
            <blockquote className="font-serif text-[1.9rem] sm:text-4xl lg:text-5xl xl:text-[3.4rem] text-cream font-normal italic leading-[1.18]">
              "Cada produto carrega<br className="hidden sm:block" /> uma história única."
            </blockquote>
            <a
              href="#"
              className="inline-flex items-center gap-2 border border-cream/25 text-cream font-sans text-[0.72rem] font-semibold tracking-[0.2em] uppercase px-8 py-4 hover:bg-cream/8 transition-colors mt-2"
            >
              Descubra Quem Faz
              <ArrowRight size={13} />
            </a>
