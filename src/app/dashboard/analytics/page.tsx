...
function computeProductsTable(items: FlatItem[]) {
  ...
  const map = new Map<string, { nome: string; categoria: string | null; foto: string | null; qty: number; receita: number }>();

  for (const i of items) {
    const key = i.produto_id ?? i.nome_produto;
    if (!map.has(key)) map.set(key, { nome: i.nome_produto, categoria: i.categoria, foto: i.foto, qty: 0, receita: 0 });
...
export default function DashboardAnalyticsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!produtora) {
        setLoadError("Produtora não encontrada. Complete seu perfil para acessar o Analytics.");
        setLoading(false);
        return;
      }

      const nomeMarca = produtora.nome_marca as string;
      setProducerName(nomeMarca);
      ...
      if (error) { console.error("[DashboardAnalytics]", error); setLoading(false); return; }

      const flat: FlatItem[] = [];

      for (const pedido of (pedidos ?? []) as any[]) {
        for (const item of (pedido.itens_pedido ?? [])) {
          if (item.produtor !== nomeMarca) continue;
  ...
  const allTimeReceita = useMemo(() => allItems.reduce((s, i) => s + i.receita, 0), [allItems]);
  const allTimePedidos = useMemo(() => new Set(allItems.map(i => i.pedido_id)).size, [allItems]);
  const allTimeTicket  = useMemo(() => allTimePedidos > 0 ? allTimeReceita / allTimePedidos : 0, [allTimeReceita, allTimePedidos]);

  const allTimeTopProd = useMemo(() => {
    const map = new Map<string, { nome: string; qty: number }>();
    for (const i of allItems) {
      const key = i.produto_id ?? i.nome_produto;
      if (!map.has(key)) map.set(key, { nome: i.nome_produto, qty: 0 });
      map.get(key)!.qty += i.quantidade;
    }
    return Array.from(map.values()).sort((a, b) => b.qty - a.qty)[0] ?? null;
  }, [allItems]);
  ...
  const filteredItems = useMemo(() => {
    ...
    return items;
  }, [allItems, preset, customFrom, customTo, filterCategoria]);
    ...
    return (
      <div className="flex flex-col gap-6">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
          {[...Array(4)].map((_, i) => <div key={i} className="h-28 bg-sand/40 border border-sand animate-pulse" />)}
        </div>
  ...
  if (loadError) {
    return (
      <div className="flex flex-col gap-2">
        <p className="font-sans text-sm text-wine">{loadError}</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <div>
        ...
        <h1 className="font-serif text-3xl text-espresso font-normal">Analytics</h1>
      </div>

      {/* ── All-time stats ──────────────────────────────────────────────────── */}
      <div>
        <p className={`${labelCls} mb-4`}>Métricas — todos os tempos</p>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
          {[
            { label: "Receita Total",         value: fmtBRL(allTimeReceita),       sub: "desde o início" },
            { label: "Pedidos",               value: String(allTimePedidos),        sub: "pedidos realizados" },
            { label: "Ticket Médio",          value: fmtBRL(allTimeTicket),         sub: "por pedido" },
            { label: "Produto Mais Vendido",  value: allTimeTopProd?.nome ?? "—",   sub: allTimeTopProd ? `${allTimeTopProd.qty} unidades` : "" },
          ].map(stat => (
            <div key={stat.label} className="bg-cream border border-sand p-5 flex flex-col gap-2">
              <p className={labelCls}>{stat.label}</p>
              <p className="font-serif text-[1.1rem] sm:text-[1.2rem] text-espresso font-medium leading-tight truncate">{stat.value}</p>
              {stat.sub && <p className="font-sans text-[0.62rem] text-espresso/35">{stat.sub}</p>}
            </div>
          ))}
        </div>
      </div>
      ...
      {/* ── Filters ─────────────────────────────────────────────────────────── */}
      <div className="bg-cream border border-sand p-6 flex flex-col gap-5">
            ...
            <table className="w-full text-left border-collapse min-w-[520px]">
              ...
              <tbody>
                {productsTable.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-5 py-16 text-center font-serif text-lg text-espresso/30 italic">
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <div className="h-1.5 bg-sand/40 w-14 flex-shrink-0">
      ...
      {categoriesData.length > 0 && (
        <div className="flex flex-col gap-5">
          <h3 className="font-serif text-xl text-espresso">Por Categoria</h3>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {categoriesData.map(cat => (
              <div key={cat.name} className="bg-cream border border-sand p-5 flex flex-col gap-4">
                <div className="flex items-center gap-2">
                  <cat.Icon size={15} strokeWidth={1.7} className="text-caramel flex-shrink-0" />
                  <span className="font-sans text-[0.75rem] font-semibold text-espresso">{cat.name}</span>
                </div>
                <div>
